...
chrome.spawn = function (options) {
	return new Promise((resolve, reject) => {
		...
		const flags = this.options.chromeFlags || [
			'--headless',
			'--disable-gpu',
		...
		if (this.options.logChromeOutput) {
			util.log('Started chrome with the following flags:', flags.join(' '));
...
chrome.kill = function () {
	...
	return new Promise((resolve, reject) => {
		this.chromeChild.on('close', () => resolve());
	...
	return new Promise((resolve, reject) => {
		...
		let connect = () => {
			CDP.Version({ host: this.options.browserDebuggingHost, port: this.options.browserDebuggingPort }).then((info) => {
				...
				util.log('Encountered error connecting to Chrome. Will retry in one second...', err);
...
chrome.openTab = function (options) {
	return new Promise((resolve, reject) => {
		...
		const connectToBrowser = async (target, host, port) => {
		...
		connectToBrowser(this.webSocketDebuggerURL, this.options.browserDebuggingHost, this.options.browserDebuggingPort)
			.then((chromeBrowser) => {
				...
				return connectToBrowser(targetId, this.options.browserDebuggingHost, this.options.browserDebuggingPort);
			}).then((tab) => {
