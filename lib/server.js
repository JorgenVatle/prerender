...
server.init = function(options, expressApp) {
	this.plugins = this.plugins || [];
	this.options = options || {};

	this.options.port = this.options.port || WEB_SERVER_PORT;
...
server.start = function() {
	...
	process.on('SIGINT', () => {
		util.log('SIGINT received. Stopping Prerender')
		this.stop();
	});
	...
	return Promise.all([
		...
		util.log('Error starting Prerender', error);
		return this.stop();
	})
};

server.stop = function() {
	util.log('Shutting down Prerender services...');
	...
	return Promise.all([
		...
		new Promise((resolve, reject) => {
			this.webServer.close((error) => {
				if (!error) {
					util.log('Express server stopped');
					return resolve();
				}

				util.log('Encountered error while shutting down Express', error);
				reject(error);
			});
		}).finally(() => {
			process.exit(0);
...
server.startExpress = function() {
	return new Promise((resolve, reject) => {
		this.webServer = this.express.listen(this.options.port, (error) => {
				if (error) {
					reject(error);
					return;
				}

				util.log(`Prerender server accepting requests on port ${this.options.port}`);
				resolve();
			}
		);
	});
